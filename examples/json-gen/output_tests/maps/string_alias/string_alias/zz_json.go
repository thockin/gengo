...
func ast_string_alias_Ttest(obj *Ttest) (libjson.Value, error) {
	return ast_Map_string_alias_StringAlias_To_string_alias_StringAlias((*map[StringAlias]StringAlias)(obj))
}

func ast_Map_string_alias_StringAlias_To_string_alias_StringAlias(obj *map[StringAlias]StringAlias) (libjson.Value, error) {
	...
	add := func(ks string) libjson.Value {
		...
		vals = append(vals, x)
		obj := &vals[len(vals)-1]
		jv, _ := ast_string_alias_StringAlias((*StringAlias)(obj))
		//FIXME: handle error?
		return jv
	}
	get := func() (map[string]libjson.Value, error) {
		...
		for k, v := range *obj {
			...
			*obj = v
			...
			jv, err := ast_string_alias_StringAlias((*StringAlias)(obj))
			if err != nil {
				return nil, err
			}
	...
	setNull := func(b bool) (libjson.Value, error) {
		...
		*obj = map[StringAlias]StringAlias{}
...
func ast_string_alias_StringAlias(obj *StringAlias) (libjson.Value, error) {
