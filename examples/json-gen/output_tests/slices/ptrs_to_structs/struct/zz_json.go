...
func ast_struct_Ttest(obj *Ttest) (libjson.Value, error) {
	return ast_Slice_Pointer_Struct_struct_Elem_struct_Elem((*[]*struct {
		F1 Elem
		F2 Elem
	})(obj))
}
...
func ast_Slice_Pointer_Struct_struct_Elem_struct_Elem(obj *[]*struct {
	F1 Elem
	F2 Elem
}) (libjson.Value, error) {
	...
	get := func() ([]libjson.Value, error) {
		...
		for i := range *obj {
			...
			jv, err := ast_Pointer_Struct_struct_Elem_struct_Elem((**struct {
				F1 Elem
				F2 Elem
			})(obj))
			if err != nil {
	...
	add := func() libjson.Value {
		...
		*obj = append(*obj, x)
		obj := &(*obj)[len(*obj)-1]
		jv, _ := ast_Pointer_Struct_struct_Elem_struct_Elem((**struct {
			F1 Elem
			F2 Elem
		})(obj))
		//FIXME: handle error?
...
func ast_Pointer_Struct_struct_Elem_struct_Elem(obj **struct {
	F1 Elem
	F2 Elem
}) (libjson.Value, error) {
	...
	setNull := func(b bool) (libjson.Value, error) {
		...
		*obj = new(struct {
		...
		obj := *obj
		return ast_Struct_struct_Elem_struct_Elem((*struct {
			F1 Elem
			F2 Elem
		})(obj))
	}
